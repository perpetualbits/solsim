...
/// How/why: `fbm = Σ amplitudeᵢ·noise(2ⁱ·p)` with the amplitude halving each
/// octave; dividing by the total amplitude keeps the result in 0..1. The doubling
/// frequency with halving strength is what makes it look the same at every scale.
/// Principle: self-similar detail — the heart of fractal textures.
/// Units: `p` dimensionless; returns a value in 0..1.
fn fbm(mut p: DVec3, octaves: u32) -> f64 {
    let mut sum = 0.0;
    let mut amp = 0.5;
    let mut norm = 0.0;
    for _ in 0..octaves {
...
mod tests {
    ...
    fn fbm_is_bounded() {
        for i in 0..50 {
            let t = i as f64;
            let n = fbm(DVec3::new(t * 0.7, t * 1.3, t * 0.2), OCTAVES);
